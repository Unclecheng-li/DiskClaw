  ...
  return String.raw`
    (() => {
      ...
      const SETTINGS_SECTIONS = [
        { key: "basic", label: "��������", copy: "�Զ�ɨ�衢֪ͨ��Ĭ��Ŀ¼��", icon: "��" },
        { key: "llm", label: "LLM ����", copy: "������ģ�ͽ�������Կ��", icon: "AI" },
        { key: "rules", label: "��������", copy: "ɨ�跶Χ���׺���������ֵ��", icon: "��" },
        { key: "system", label: "ϵͳ����", copy: "���ڡ����ԡ������������", icon: "ϵ" },
        { key: "data", label: "���ݹ���", copy: "���ݡ��ָ����������á�", icon: "��" },
        { key: "schedule", label: "�ƻ�����", copy: "ϵͳ���û��ƻ����������", icon: "��" },
        { key: "update", label: "Ӧ�ø���", copy: "���汾��������ӡ�", icon: "��" },
        { key: "advanced", label: "�߼�����", copy: "��־�������������Ϣ��", icon: "��" }
      ];
      ...
      function fileNameFromPath(filePath) {
        ...
        return parts[parts.length - 1] || String(filePath || "未命名文�?);
      }
      ...
      function typeLabel(type) {
        const map = {
          ...
          "hotspot-cleanup": "不常用目录清�?,
          restore: "隔离恢复",
          "quarantine-delete": "隔离区删�?,
          "quarantine-clear": "隔离区清�?,
      ...
      function formatScheduleSource(source) {
        return source === "system-preferences" ? "系统自动计划" : "用户自定义计�?;
      }
      ...
      function formatUpdateState(update) {
        ...
        if (!update.ok) {
          return update.reason || "检查失�?;
        }

        return update.updateAvailable
          ? "发现新版�?" + update.latestVersion
          : "当前已是最新版�?" + update.currentVersion;
      }
      ...
      function getHomeFooterText() {
        ...
        if (latestCleanup) {
          ...
          const daysText = days === null ? "最�? : days === 0 ? "今天" : (days + " 天前");
      ...
      function renderNavigation() {
        const items = [
          { view: "home", icon: "��", label: "��ҳ" },
          { view: "quarantine", icon: "��", label: "������" },
          { view: "history", icon: "��", label: "������ʷ" },
          { view: "settings", icon: "��", label: "����" }
        ];
      ...
      function renderTopbar() {
        ...
        const llmStatus = appState.health.llm?.ok
          ? '<span class="status-dot"></span>AI ������'
          : '<span class="status-dot ' + (appState.health.ready ? "warn" : "") + '"></span>' + escapeHtml(appState.health.llm?.reason || "AI δ����");
        ...
        topbarMount.innerHTML =
          ...
          '<div class="topbar-actions">' +
            ...
            '<div class="status-chip"><span class="status-dot ' + (target ? "" : "warn") + '"></span>' + escapeHtml(target ? ("��ǰĿ¼��" + truncateMiddle(target, 30)) : "��δѡ��ɨ��λ��") + "</div>" +
            '<button class="button-subtle" data-action="refresh-data">ˢ������</button>' +
          "</div>";
      }
      ...
      function renderTaskBanner() {
        ...
        const statusText = task.status === "paused" ? "已暂�? : task.status === "cancelled" ? "已取�? : task.status === "failed" ? "执行失败" : "执行�?;
        const details = task.details || {};
        const meta = [];

        if (typeof details.scannedFiles === "number") {
          meta.push("已扫�?" + details.scannedFiles + " 个文�?);
        }
        ...
        if (typeof details.completedItems === "number") {
          meta.push("已处�?" + details.completedItems + " / " + (details.totalItems || 0) + " 个文�?);
        }

        if (typeof details.releasedBytes === "number") {
          meta.push("已释�?" + formatBytes(details.releasedBytes));
        }
            ...
            <div class="home-stack">
              ...
              <button class="hero-orb scan-button" data-action="start-home-scan">
                <div class="hero-orb-state">
                  <div class="hero-orb-mark">�?/div>
                  <div class="hero-orb-label">\${target ? "一键智能扫�? : "选择文件夹开�?}</div>
                  <div class="hero-orb-note">\${target ? "打开软件后，三步完成扫描与清�? : "先选择一个常用文件夹"}</div>
                </div>
              </button>
              <div class="hero-kpis">
                ...
                <div class="hero-kpi-card">
                  ...
                  <strong>\${appState.health.llm?.ok ? "AI 已连�? : "本地规则可用"}</strong>
                  <span>\${escapeHtml(appState.health.llm?.ok ? "会结�?LLM 生成更易理解的清理建议�? : "即使未配�?LLM，也能先完成本地扫描和基础判断�?)}</span>
                </div>
              </div>
                ...
                <div class="target-card \${target ? "active" : ""}">
                  <div class="target-card-head">
                    ...
                    <div class="inline-actions">
                      <button class="button-subtle" data-action="browse-target">选择文件�?/button>
                    </div>
                  </div>
            ...
            <div class="home-stack">
              ...
              <div class="hero-orb progress" style="--progress:\${Math.max(6, appState.home.progress || 0)};">
                <div class="hero-orb-state">
                  ...
                  <div class="hero-orb-label">正在扫描�?..</div>
              ...
              <div class="summary-grid">
                ...
                <div class="metric-card">
                  ...
                  <div class="metric-value">\${escapeHtml(fileNameFromPath(target || stats.currentPath || "未设�?))}</div>
                  <div class="metric-copy">正在逐层检查文件与子目录�?/div>
                </div>
                <div class="metric-card">
                  <div class="metric-label">后台执行</div>
                  <div class="metric-value">已开�?/div>
          ...
          <section class="card home-hero">
            <div class="home-stack" style="width:min(980px, 100%);">
              <div class="hero-summary">
                ...
                <div class="home-actions">
                  <button class="button-primary warm" data-action="start-home-cleanup" \${cleanupItems.length ? "" : "disabled"}>一键清�?/button>
                ...
                <div class="split-layout">
                  <div class="story-card">
                    ...
                    <div class="summary-grid">
                      ...
                      <div class="metric-card">
                        <div class="metric-label">重复文件�?/div>
                        <div class="metric-value">\${(summary.duplicateGroups || []).length}</div>
                      </div>
                      <div class="metric-card">
                        <div class="metric-label">不常用目�?/div>
                      ...
                      \${priorities.length ? priorities.slice(0, 3).map((item) => \`
                        <div class="priority-item" data-action="switch-view" data-view="history">
                          <div class="priority-head">
                            <h4 class="priority-title">\${escapeHtml(item.title || "优先处理�?)}</h4>
            ...
            <div class="home-stack">
              ...
              <div class="story-card" style="text-align:left;">
                ...
                <div class="summary-grid" style="margin-top:18px;">
                  <div class="metric-card">
                    <div class="metric-label">已清理文�?/div>
                  ...
                  <div class="metric-card">
                    <div class="metric-label">已释放空�?/div>
              ...
              <div class="home-actions">
                ...
                <button class="button-subtle" data-action="switch-view" data-view="quarantine">查看隔离�?/button>
              </div>
            </div>
          </section>
        \`;
          ...
          <section class="card home-hero">
            <div class="home-stack">
              ...
              <div>
                ...
                <p class="home-subtitle">本次共释�?\${formatBytes(summary.releasedBytes)}，清理了 \${summary.count} 个文件。所有结果都已经进入隔离区或按您的执行方式处理完成�?/p>
              </div>
              <div class="summary-grid">
                <div class="metric-card">
                ...
                <div class="metric-card">
                  ...
                  <div class="metric-value">\${escapeHtml(result?.reportId || "已生�?)}</div>
                </div>
              </div>
              <div class="story-card">
                <h3 class="story-title">本次收尾提示</h3>
                <p class="story-copy">如果你只是想确认结果是否安全，可以先去隔离区抽查几项；如果你想复盘本次清理收益，可以直接查看详细报告�?/p>
              </div>
              <div class="home-actions">
        ...
        return \`
          <section class="card">
            <div class="toolbar">
              <div>
                <h2 class="section-title">隔离�?/h2>
                <p class="section-copy">共隔�?\${activeItems.length} 个文件，占用 \${formatBytes(totalBytes)} 空间。隔离文件将�?\${retention} 天后自动永久删除�?/p>
              </div>
              <div class="toolbar-end">
                <input id="quarantineSearchInput" class="toolbar-select" value="\${escapeHtml(appState.quarantine.search)}" placeholder="搜索文件�?/ 路径 / 时间">
                <select class="toolbar-select" id="quarantineStatusSelect">
                  <option value="all" \${selected(appState.quarantine.statusFilter, "all")}>全部状�?/option>
                  <option value="active" \${selected(appState.quarantine.statusFilter, "active")}>待恢�?/option>
                  <option value="restored" \${selected(appState.quarantine.statusFilter, "restored")}>已恢�?/option>
                </select>
                <select class="toolbar-select" id="quarantineSortSelect">
                  <option value="date-desc" \${selected(appState.quarantine.sort, "date-desc")}>按隔离时间排�?/option>
                  <option value="size-desc" \${selected(appState.quarantine.sort, "size-desc")}>按文件大小排�?/option>
                  <option value="name-asc" \${selected(appState.quarantine.sort, "name-asc")}>按文件名排序</option>
                </select>
                <button class="button-secondary" data-action="restore-selected" \${appState.quarantine.selectedIds.length ? "" : "disabled"}>恢复选中�?/button>
                <button class="button-warn" data-action="delete-selected" \${appState.quarantine.selectedIds.length ? "" : "disabled"}>永久删除选中�?/button>
                <button class="button-warn" data-action="clear-quarantine" \${activeItems.length ? "" : "disabled"}>清空隔离�?/button>
              </div>
            </div>
            <div class="summary-grid" style="margin-bottom:18px;">
              <div class="metric-card">
                <div class="metric-label">隔离文件�?/div>
              ...
              <div class="metric-card">
                <div class="metric-label">已选文�?/div>
                ...
                <div class="quarantine-head">
                  <div class="checkbox-row grow">
                    ...
                    <div class="quarantine-main">
                      ...
                      <p class="history-summary">批量恢复适合误删复原，批量永久删除会直接释放隔离区占用空间。当前筛选结果共 \${visibleItems.length} 项�?/p>
                    </div>
                  </div>
                </div>
                <div class="history-actions">
                  <span class="chip">\${selectedSummary.count} 已�?/span>
                      ...
                      <div class="checkbox-row grow">
                        ...
                        <div class="quarantine-main">
                          ...
                          <div class="quarantine-meta">
                            \${makeChip(item.restoredAt ? "已恢�? : "待恢�?, item.restoredAt ? "" : "low")}
            ...
            <div class="toolbar">
              ...
              <div class="toolbar-end">
                ...
                <select class="toolbar-select" id="historyRangeSelect">
                  ...
                  <option value="7" \${selected(appState.reportRange, "7")}>最�?7 �?/option>
                  <option value="30" \${selected(appState.reportRange, "30")}>最�?30 �?/option>
                </select>
                ...
                <select class="toolbar-select" id="historySortSelect">
                  <option value="date-desc" \${selected(appState.reportSort, "date-desc")}>按时间排�?/option>
                  <option value="space-desc" \${selected(appState.reportSort, "space-desc")}>按释放空间排�?/option>
                    ...
                    <div class="history-title-block">
                      <div class="history-select-row">
                        ...
                        <div>
                          ...
                          <p class="history-summary">可在这里批量选择、查看、导出或删除报告。长列表会在模块内部滚动，不会拉长整个页�?�?/p>
                        </div>
                      </div>
                    </div>
                    <div class="history-actions">
                      <span class="chip">\${appState.selectedReports.length} 已�?/span>
                    ...
                    <div class="history-head">
                      <div class="history-title-block">
                        ...
                        <div class="history-meta">
                          ...
                          \${report.itemCount ? makeChip("文件�?" + report.itemCount) : ""}
      ...
      function renderSettingsView() {
        ...
        return \`
          <section class="settings-grid">
            \${renderSettingSection("basic", "基础设置", "普通用户最常用的配置都集中在这里�?, \`
              <div class="story-card" style="margin-bottom:18px;">
                <h3 class="story-title">当前自动计划摘要</h3>
                \${systemSchedule ? \`
                  <div class="summary-grid" style="margin-top:14px;">
                    <div class="metric-card">
                      <div class="metric-label">计划状�?/div>
                      <div class="metric-value">已启�?/div>
                      <div class="metric-copy">\${escapeHtml(systemSchedule.name || "自动扫描")}</div>
                    </div>
                    <div class="metric-card">
                      <div class="metric-label">执行频率</div>
                    ...
                    <div class="metric-card">
                      <div class="metric-label">执行动作</div>
                      <div class="metric-value">\${escapeHtml(systemSchedule.action === "auto-cleanup" ? "自动清理" : "自动扫描")}</div>
                      <div class="metric-copy">\${escapeHtml(summarizeScheduleAction(systemSchedule.action))}</div>
                    </div>
                    <div class="metric-card">
                      <div class="metric-label">扫描目录</div>
                      <div class="metric-value">\${escapeHtml(fileNameFromPath(systemSchedule.targets?.[0]?.path || "未设�?))}</div>
                      <div class="metric-copy" title="\${escapeHtml(systemSchedule.targets?.[0]?.path || "")}">\${escapeHtml(truncateMiddle(systemSchedule.targets?.[0]?.path || "未设�?, 36))}</div>
                    </div>
                  </div>
                  <div class="button-row" style="margin-top:14px;">
                    <button class="button-secondary" data-action="run-system-schedule-now" data-schedule-id="\${escapeHtml(systemSchedule.id)}">立即执行一�?/button>
                    <button class="button-subtle" data-action="refresh-system-schedule">刷新自动计划</button>
                  </div>
                \` : \`
              ...
              <div class="form-grid">
                ...
                <label class="form-row">
                  ...
                  <select id="prefAutoScanInterval">
                    <option value="daily" \${selected(pref.autoScanInterval, "daily")}>每天</option>
                    <option value="weekly" \${selected(pref.autoScanInterval, "weekly")}>每周</option>
                    <option value="monthly" \${selected(pref.autoScanInterval, "monthly")}>每月</option>
                  </select>
                </label>
                <label class="form-row">
                  ...
                  <div class="checkbox-row">
                    ...
                    <span>扫描完成后自动清理低风险�?/span>
                  </div>
                </label>
                <label class="form-row">
                  <span class="label">隔离文件保留时间</span>
                  <select id="prefRetentionDays">
                    <option value="7" \${selected(String(pref.quarantineRetentionDays), "7")}>7 �?/option>
                    <option value="15" \${selected(String(pref.quarantineRetentionDays), "15")}>15 �?/option>
                    <option value="30" \${selected(String(pref.quarantineRetentionDays), "30")}>30 �?/option>
                    <option value="9999" \${selected(String(pref.quarantineRetentionDays), "9999")}>永久</option>
                  </select>
                </label>
              </div>
              ...
              <div class="button-row" style="margin-top:18px;">
                <button class="button-primary" data-action="save-basic-settings">保存基础设置</button>
              </div>
            \`)}
            \${renderSettingSection("llm", "LLM 配置", "默认折叠，只有在需要接入第三方模型时再展开修改�?, \`
              <p class="panel-tip">修改此配置可能影�?AI 分析准确性，非必要请勿修改�?/p>
              <div class="form-grid" style="margin-top:14px;">
                <label class="form-row">
                  <span class="label">启用 AI 分析</span>
                  <div class="checkbox-row">
                    <input id="llmEnabled" type="checkbox" \${checked(llm.enabled)}>
                    <span>允许调用外部 LLM 接口</span>
                  </div>
                </label>
                <label class="form-row">
                ...
                <label class="form-row">
                  ...
                  <input id="llmApiKey" value="" placeholder="\${escapeHtml(llm.hasApiKey ? "已保存，如需更换请重新输�? : "sk-...")}">
                </label>
                <label class="form-row">
              ...
              <div class="button-row" style="margin-top:18px;">
                <button class="button-subtle" data-action="load-llm-config">读取已保存配�?/button>
                <button class="button-subtle" data-action="fill-llm-defaults">使用默认配置</button>
                <button class="button-secondary" data-action="test-llm-config">测试连接</button>
                <button class="button-primary" data-action="save-llm-config">保存配置</button>
              </div>
            \`)}
            \${renderSettingSection("rules", "规则配置", "保留 v1.0 的规则能力，并给出推荐配置入口�?, \`
              <p class="panel-tip">修改此配置可能导致误删重要文件，请谨慎操作�?/p>
              <div class="form-grid" style="margin-top:14px;">
                <label class="form-row">
                  <span class="label">默认扫描目录</span>
              ...
              <div class="form-grid single" style="margin-top:14px;">
                <label class="form-row">
                  <span class="label">排除路径</span>
                  <textarea id="rulesExcludePaths" placeholder="每行一个路�?>\${escapeHtml((rules.excludePaths || []).join("\\n"))}</textarea>
                </label>
                <label class="form-row">
                  <span class="label">白名单路�?/span>
                  <textarea id="rulesWhitelistPaths" placeholder="每行一个路�?>\${escapeHtml((rules.whitelistPaths || []).join("\\n"))}</textarea>
                </label>
                <label class="form-row">
                  <span class="label">黑名单路�?/span>
                  <textarea id="rulesBlacklistPaths" placeholder="每行一个路�?>\${escapeHtml((rules.blacklistPaths || []).join("\\n"))}</textarea>
                </label>
              </div>
              <div class="button-row" style="margin-top:18px;">
                <button class="button-subtle" data-action="load-rules">读取规则</button>
                <button class="button-subtle" data-action="reset-rules">恢复默认规则</button>
                <button class="button-primary" data-action="save-rules">保存规则配置</button>
              </div>
            \`)}
            \${renderSettingSection("system", "系统设置", "窗口、通知、语言与更新偏好都放在这里�?, \`
              <div class="form-grid">
                <label class="form-row">
                  <span class="label">开机自动启�?/span>
                  <div class="checkbox-row">
                ...
                <label class="form-row">
                  <span class="label">关闭窗口�?/span>
                  <select id="sysCloseBehavior">
                    <option value="ask" \${selected(desktop.closeBehavior, "ask")}>每次询问</option>
                    <option value="tray" \${selected(desktop.closeBehavior, "tray")}>最小化到托�?/option>
                  </select>
                </label>
                <label class="form-row">
                  <span class="label">总是置顶</span>
                  <div class="checkbox-row">
                ...
                <label class="form-row">
                  <span class="label">记住窗口大小</span>
                  <div class="checkbox-row">
                    <input id="sysRememberBounds" type="checkbox" \${checked(pref.rememberWindowBounds)}>
                    <span>重启后恢复上次窗口尺�?/span>
                  </div>
                </label>
                <label class="form-row">
                  <span class="label">语言</span>
                  <select id="sysLanguage">
                    <option value="zh-CN" \${selected(pref.language, "zh-CN")}>简体中�?/option>
                    <option value="en" \${selected(pref.language, "en")}>English</option>
                  </select>
                </label>
                <label class="form-row">
                  <span class="label">检查更�?/span>
                  <select id="sysCheckUpdates">
                    <option value="auto" \${selected(pref.checkUpdates, "auto")}>自动检�?/option>
                    <option value="manual" \${selected(pref.checkUpdates, "manual")}>手动检�?/option>
                  </select>
                </label>
                <label class="form-row">
                  <span class="label">更新清单地址</span>
                  <input id="updateManifestUrl" value="\${escapeHtml(pref.updateManifestUrl || "")}" placeholder="https://example.com/diskclaw-update.json">
                </label>
              </div>
              <div class="button-row" style="margin-top:18px;">
                <button class="button-subtle" data-action="load-desktop-settings">读取桌面设置</button>
                <button class="button-secondary" data-action="test-notification">测试系统通知</button>
                <button class="button-primary" data-action="save-system-settings">保存系统设置</button>
              </div>
            \`)}
            \${renderSettingSection("data", "数据管理", "备份配置、恢复配置、重置所有设置�?, \`
              <div class="story-card">
                <h3 class="story-title">备份与恢�?/h3>
                <p class="story-copy">你可以把当前配置导出成一�?JSON 备份，也可以把备份恢复回当前设备。重置会清除所有配置并恢复默认值�?/p>
                <div class="button-row" style="margin-top:16px;">
                  <button class="button-secondary" data-action="backup-config">备份配置</button>
                  <button class="button-subtle" data-action="restore-config">恢复配置</button>
                  <button class="button-warn" data-action="reset-all-config">重置所有设�?/button>
                </div>
                <p class="story-copy" style="margin-top:12px;">隔离文件会按“保留时间”自动清理，当前设置中的自动计划会持续同步�?/p>
              </div>
            \`) }
          </section>
          <section class="card" style="margin-top:20px;">
            <h2 class="section-title">计划任务</h2>
            <p class="section-copy">系统自动计划来自基础设置，用户计划则来自手动配置。两者分开展示，避免混淆�?/p>
            <div class="split-layout" style="margin-top:18px;">
              ...
              <div class="story-card">
                <h3 class="story-title">用户自定义计�?/h3>
                <div class="button-row" style="margin-top:14px;">
                  <button class="button-primary" data-action="new-user-schedule">新建用户计划</button>
                  ...
                  <div class="story-card" style="margin-top:14px;">
                    ...
                    <div class="form-grid" style="margin-top:14px;">
                      <label class="form-row">
                        ...
                        <input id="userScheduleName" value="\${escapeHtml(appState.settings.scheduleEditor.name || "")}" placeholder="例如：每周下载目录扫�?>
                      </label>
                      <label class="form-row">
                      ...
                      <label class="form-row">
                        ...
                        <select id="userScheduleInterval">
                          <option value="1440" \${selected(String(appState.settings.scheduleEditor.intervalMinutes), "1440")}>每天</option>
                          <option value="10080" \${selected(String(appState.settings.scheduleEditor.intervalMinutes), "10080")}>每周</option>
                          <option value="43200" \${selected(String(appState.settings.scheduleEditor.intervalMinutes), "43200")}>每月</option>
                        </select>
                      </label>
                      <label class="form-row">
                        <span class="label">执行动作</span>
                        <select id="userScheduleAction">
                          <option value="plan-only" \${selected(appState.settings.scheduleEditor.action, "plan-only")}>仅扫�?/option>
                          <option value="auto-cleanup" \${selected(appState.settings.scheduleEditor.action, "auto-cleanup")}>自动清理低风险项</option>
                        </select>
                      </label>
                      <label class="form-row">
                        <span class="label">清理模式</span>
                        <select id="userScheduleCleanupMode">
                          <option value="quarantine" \${selected(appState.settings.scheduleEditor.cleanupMode, "quarantine")}>隔离�?/option>
                          <option value="archive" \${selected(appState.settings.scheduleEditor.cleanupMode, "archive")}>归档</option>
                          <option value="recycle-bin" \${selected(appState.settings.scheduleEditor.cleanupMode, "recycle-bin")}>回收�?/option>
                          <option value="permanent" \${selected(appState.settings.scheduleEditor.cleanupMode, "permanent")}>永久删除</option>
                        </select>
                      </label>
                      <label class="form-row">
                        <span class="label">最大处理项</span>
                        <input id="userScheduleMaxItems" type="number" value="\${Number(appState.settings.scheduleEditor.maxItems || 10)}">
                      </label>
                    </div>
                    <div class="form-grid" style="margin-top:14px;">
                      <label class="form-row">
                        <span class="label">计划状�?/span>
                        <div class="checkbox-row">
                      ...
                      <label class="form-row">
                        ...
                        <div class="checkbox-row">
                          ...
                          <span>仅模拟执行，不真正清�?/span>
                        </div>
                      </label>
                    </div>
                    <div class="button-row" style="margin-top:18px;">
                      <button class="button-subtle" data-action="browse-user-schedule-target">选择目录</button>
                      <button class="button-primary" data-action="save-user-schedule">\${appState.settings.scheduleEditor.id ? "保存修改" : "创建计划"}</button>
                    </div>
                  </div>
                \` : ""}
                  ...
                  <div class="history-list" style="margin-top:14px;">
                    \${userSchedules.map((item) => \`
                      <div class="history-item">
                        <div class="history-head">
                          <div>
                            <h4 class="history-title">\${escapeHtml(item.name || "未命名计�?)}</h4>
                            <div class="history-meta">
                              \${makeChip(formatScheduleSource(item.source))}
                              \${makeChip(formatIntervalMinutes(item.intervalMinutes))}
                              \${makeChip(item.action === "auto-cleanup" ? "自动清理" : item.action === "plan-only" ? "仅扫�? : item.action)}
                            </div>
                          </div>
                          <div class="inline-actions">
          ...
          <section class="card" style="margin-top:20px;">
            ...
            <p class="section-copy">你可以手动检查版本，也可以开启自动检查。更新检查结果会显示在这里，并可直接打开下载页�?/p>
            <div class="summary-grid" style="margin-top:18px;">
              <div class="metric-card">
                <div class="metric-label">当前版本</div>
                <div class="metric-value">v\${escapeHtml(appState.settings.appVersion || "0.1.0")}</div>
                <div class="metric-copy">与安装包版本保持一致�?/div>
              </div>
              <div class="metric-card">
                <div class="metric-label">更新状�?/div>
                <div class="metric-value">\${escapeHtml(formatUpdateState(update))}</div>
                <div class="metric-copy">\${escapeHtml(update?.downloadUrl ? "可直接打开下载页�? : "请先配置更新清单地址�?)}</div>
              </div>
              <div class="metric-card">
                <div class="metric-label">自动检�?/div>
                <div class="metric-value">\${pref.checkUpdates === "auto" ? "开�? : "关闭"}</div>
                <div class="metric-copy">保存后会在启动时和定时轮询时检查�?/div>
              </div>
              <div class="metric-card">
                <div class="metric-label">更新地址</div>
            ...
            <div class="button-row" style="margin-top:18px;">
              <button class="button-secondary" data-action="check-update-now">立即检查更�?/button>
              <button class="button-subtle" data-action="open-update-url" \${update?.downloadUrl ? "" : "disabled"}>打开下载�?/button>
            </div>
            <div class="story-card" style="margin-top:16px;">
              <h3 class="story-title">更新说明</h3>
              <p class="story-copy">\${escapeHtml(update?.releaseNotes || "暂无更新说明�?)}</p>
            </div>
          </section>
          <section class="card" style="margin-top:20px;">
            \${renderSettingSection("advanced", "高级设置", "日志与调试能力默认折叠，避免打扰普通用户�?, \`
              <div class="log-toolbar">
                <input id="logSearch" placeholder="搜索日志关键�?>
                ...
                <select id="logTimeFilter">
                  ...
                  <option value="5m">最�?5 分钟</option>
                  <option value="1h">最�?1 小时</option>
                  <option value="24h">最�?24 小时</option>
                </select>
              ...
              <div class="button-row" style="margin-bottom:14px;">
                <button class="button-subtle" data-action="export-logs" data-format="txt">导出 TXT</button>
                <button class="button-subtle" data-action="export-logs" data-format="json">导出 JSON</button>
                <button class="button-warn" data-action="clear-logs">清空日志</button>
                <button class="button-subtle" data-action="toggle-debug-mode">\${appState.debugMode ? "关闭调试模式" : "开启调试模�?}</button>
                <button class="button-warn" data-action="reset-local-ui">重置界面设置</button>
              </div>
          ...
          <section class="card" style="margin-top:20px;">
            ...
            <p class="section-copy">磁盘清理大虾 v2.0 正在从开发者工作台升级为面向普通用户的简约桌面产品。本次改造完全复用现有技术栈，不引入新的前端框架�?/p>
          </section>
        \`;
      }
      ...
      function enhanceSettingsLayout() {
        ...
        nav.innerHTML = \`
          <section class="card settings-nav-card">
            <h3 class="settings-nav-title">���ò˵�</h3>
      ...
      function openConfirmDialog(config) {
        ...
        titleNode.textContent = config.title || "请确认操�?;
        messageNode.textContent = config.message || "此操作会修改本地数据，请确认后继续�?;
        impactNode.textContent = config.impact || "�?;
        ...
        keywordInput.placeholder = config.keyword ? ("请输入�? + config.keyword + "�?) : "请输入确认文�?;
      ...
      function renderReportDialog() {
        ...
        content.innerHTML =
          '<div class="dialog-head">' +
            ...
            "</div>" +
            '<div class="inline-actions">' +
              '<button class="button-subtle" data-action="view-adjacent-report" data-direction="prev" ' + (prevReportId ? "" : "disabled") + ">上一�?/button>" +
              '<button class="button-subtle" data-action="view-adjacent-report" data-direction="next" ' + (nextReportId ? "" : "disabled") + ">下一�?/button>" +
              '<button class="button-subtle" data-action="export-report" data-report-id="' + escapeHtml(report.reportId) + '" data-format="json">导出 JSON</button>' +
              '<button class="button-subtle" data-action="export-report" data-report-id="' + escapeHtml(report.reportId) + '" data-format="md">导出 Markdown</button>' +
          ...
          '<div class="dialog-body">' +
            '<div class="report-grid summary-grid">' +
              ...
              '<div class="metric-card"><div class="metric-label">候选项 / 文件�?/div><div class="metric-value">' + String(candidateSummary.totalCandidates || executionSummary.count || 0) + "</div></div>" +
      ...
      function renderScheduleDialog() {
        ...
        content.innerHTML =
          ...
          '<div class="dialog-body">' +
            '<div class="summary-grid">' +
              '<div class="metric-card"><div class="metric-label">计划名称</div><div class="metric-value">' + escapeHtml(schedule.name || "未命名计�?) + "</div></div>" +
              ...
              '<div class="metric-card"><div class="metric-label">执行方式</div><div class="metric-value">' + escapeHtml(schedule.action === "auto-cleanup" ? "自动清理" : "仅扫�?) + "</div></div>" +
            ...
            '<div class="story-card"><h4 class="story-title">目录与参�?/h4><p class="story-copy">扫描目录�? + escapeHtml(schedule.targets?.[0]?.path || "未设�?) + '</p><p class="story-copy">清理模式�? + escapeHtml(schedule.cleanupMode || "quarantine") + '，最大处理项�? + String(schedule.maxItems || 0) + '</p></div>' +
      ...
      async function deleteReports(reportIds) {
        ...
        const result = await confirmAction({
          ...
          impact: "本次将删�?" + summary.count + " 条报告，关联空间记录�?" + formatBytes(summary.reclaimableBytes) + "�?,
          confirmLabel: "删除报告"
        });

        if (!result.confirmed) {
          showToast("warn", "已取消操�?, "没有删除任何报告�?);
          return;
        }
        ...
        showToast("success", "报告已删�?, "选中的报告已经移除�?);
        logEvent("已删除报�?, deleted, "success");
      }
      ...
      function handleExternalOpenPath(pathValue) {
        ...
        switchView("home");
        showToast("info", "已载入扫描目�?, "外部路径已准备好，可以直接开始扫描�?);
      }
      ...
      async function pollActiveTask() {
        ...
        if (task.status === "completed") {
          ...
          if (task.type === "plan") {
            ...
            logEvent("清理任务已完�?, summary, "success");
            showToast("success", "清理完成", "本次共释�?" + formatBytes(summary.releasedBytes) + " 空间�?);
          }
        ...
        if (task.status === "cancelled") {
          ...
          appState.activeTask = null;
          appState.home.status = appState.home.plan ? "scanComplete" : "idle";
          showToast("warn", "任务已取�?, "当前后台任务已停�?�?);
          logEvent("后台任务已取�?, { type: task.type }, "warn");
          renderApp();
        }
      }
      ...
      function startTaskPolling() {
        ...
        pollActiveTask().catch((error) => {
          showRequestErrorToast(error, "任务状态刷新失�?);
        });
      ...
      async function browseTarget() {
        if (!window.diskClawDesktop?.chooseFolder) {
          showToast("warn", "当前环境不支�?, "浏览文件夹仅在桌面端可用�?);
          return;
        }
        ...
        logEvent("已更新扫描目�?, { path: selectedPath }, "success");
        renderTopbar();
        if (appState.currentView === "home") {
          renderCurrentView();
        }
      ...
      async function startHomeScan() {
        ...
        logEvent("已开始智能扫�?, { path: scanTarget }, "info");
        renderApp();
        startTaskPolling();
      }
      ...
      async function startHomeCleanup() {
        ...
        if (!items.length) {
          showToast("warn", "暂无可执行项", "当前扫描结果中没有适合一键清理的低风险文件�?);
          return;
        }
        ...
        const result = await confirmAction({
          title: "确认开始一键清�?,
          message: "系统会优先处理低风险项，并先移动到隔离区�?,
          impact: "本次将处�?" + items.length + " 个文件，预计释放 " + formatBytes(totalBytes) + " 空间�?,
          confirmLabel: "开始清�?
        });

        if (!result.confirmed) {
          showToast("warn", "已取消操�?, "本次清理没有开始�?);
          return;
        }
        ...
        logEvent("已开始一键清�?, { itemCount: items.length }, "info");
        renderApp();
        startTaskPolling();
      }
      ...
      async function pauseTask() {
        ...
        logEvent("已请求暂停任�?, { taskId: appState.activeTaskId }, "warn");
      }
      ...
      async function resumeTask() {
        ...
        logEvent("已请求继续任�?, { taskId: appState.activeTaskId }, "info");
      }
      ...
      async function cancelTask() {
        ...
        logEvent("已请求取消任�?, { taskId: appState.activeTaskId }, "warn");
      }
      ...
      async function restoreQuarantineItems(items) {
        const result = await confirmAction({
          ...
          impact: "本次将恢�?" + items.length + " 个文件�?,
          confirmLabel: "开始恢�?
        });

        if (!result.confirmed) {
          showToast("warn", "已取消操�?, "没有恢复任何文件�?);
          return;
        }
        ...
        renderCurrentView();
        logEvent("已恢复隔离文�?, { count: restored.results?.length || 0 }, "success");
        showToast("success", "恢复完成", "选中的文件已经恢复到原路径�?);
      }

      async function deleteQuarantineItems(items, isClearAll = false) {
        const result = await confirmAction({
          title: isClearAll ? "确认清空隔离�? : "确认永久删除隔离文件",
          message: "此操作不可恢复，请确认后继续�?,
          impact: isClearAll ? "将清空所有未恢复的隔离文件�? : "本次将永久删�?" + items.length + " 个文件�?,
          confirmLabel: isClearAll ? "清空隔离�? : "永久删除"
        });

        if (!result.confirmed) {
          showToast("warn", "已取消操�?, "没有删除任何隔离文件�?);
          return;
        }
        ...
        await Promise.allSettled([loadQuarantine(), loadReports()]);
        renderCurrentView();
        logEvent(isClearAll ? "已清空隔离区" : "已永久删除隔离文�?, { count: items.length }, "success");
      ...
      async function exportReport(reportId, format) {
        ...
        showToast("success", "报告已导�?, "文件已导出到 " + exported.exportPath);
        logEvent("已导出报�?, exported, "success");
      }
      ...
      async function saveBasicSettings() {
        ...
        renderCurrentView();
        showToast("success", "基础设置已保�?, "新的偏好设置已经生效�?);
      ...
      async function saveLlmConfig() {
        ...
        await refreshHealth();
        renderCurrentView();
        showToast("success", "AI 配置已保�?, "新的 LLM 配置已经更新�?);
        logEvent("已保�?LLM 配置", {
      ...
      async function testLlmConfig() {
        ...
        logEvent("已测�?LLM 连接", result, result.ok ? "success" : "warn");
      }
      ...
      async function saveRules() {
        ...
        await loadSchedules();
        showToast("success", "规则已保�?, "扫描参数和路径规则已更新�?);
        logEvent("已保存规则配�?, appState.settings.rules, "success");
      }
      ...
      async function saveSystemSettings() {
        ...
        renderCurrentView();
        showToast("success", "系统设置已保�?, "窗口与系统偏好已经更新�?);
        logEvent("已保存系统设�?, {
          preferences,
      ...
      async function backupConfig() {
        ...
        showToast("success", "配置已备�?, "备份文件已经下载�?);
        logEvent("已备份配�?, backup, "success");
      }
      ...
      async function restoreConfig() {
        ...
        rerenderIfView("settings");
        showToast("success", "配置已恢�?, "备份配置已应用�?);
        logEvent("已恢复配�?, restored, "success");
      }
      ...
      async function resetAllConfig() {
        ...
        rerenderIfView("settings");
        showToast("success", "已重置所有设�?, "系统已经恢复为默认状态�?);
        logEvent("已重置所有设�?, reset, "success");
        scheduleAutoUpdateCheck();
      }
      ...
      async function checkUpdateNow() {
        ...
        if (result.ok) {
          showToast(result.updateAvailable ? "warn" : "success", "更新检查完�?, formatUpdateState(result));
        } else {
          showToast("warn", "更新检查失�?, result.reason || "无法检查更新�?);
        }
      }
      ...
      async function openUpdateUrl() {
        ...
        if (!window.diskClawDesktop?.openExternal) {
          showToast("warn", "当前环境不支�?, "无法打开外部链接�?);
          return;
        }
      ...
      async function runScheduleNow(scheduleId) {
        ...
        closeScheduleDialog();
        switchView("home");
        showToast("success", "自动计划已启�?, "任务已经在后台开始执行�?);
        logEvent("已手动触发计划任�?, {
          scheduleId
        }, "success");
      }
      ...
      function viewUserSchedule(scheduleId) {
        const schedule = (appState.settings.schedules || []).find((item) => item.id === scheduleId) || null;
        if (!schedule) {
          showToast("warn", "计划不存�?, "没有找到对应的用户计划�?);
          return;
        }
      ...
      async function saveUserSchedule() {
        ...
        appState.settings.scheduleEditor = null;
        rerenderIfView("settings");
        showToast("success", current.id ? "计划已更�? : "计划已创�?, "用户自定义计划已经保存�?);
        logEvent(current.id ? "已更新用户计�? : "已创建用户计�?, {
          scheduleId: saved.id,
          name: saved.name
        }, "success");
      }
      ...
      async function deleteUserSchedule(scheduleId) {
        ...
        const result = await confirmAction({
          ...
          impact: "计划名称�? + (schedule?.name || "未命名计�?),
          confirmLabel: "删除计划"
        });

        if (!result.confirmed) {
          showToast("warn", "已取消操�?, "用户计划没有被删除�?);
          return;
        }
        ...
        closeScheduleDialog();
        rerenderIfView("settings");
        showToast("success", "计划已删�?, "用户自定义计划已经移除�?);
        logEvent("已删除用户计�?, {
          scheduleId,
      ...
      function resetLocalUi() {
        ...
        showToast("success", "界面设置已重�?, "本地扫描参数和界面状态已恢复默认�?);
        logEvent("已重置本地界面设�?, null, "success");
        renderApp();
      }
      ...
      function toggleDebugMode() {
        ...
        showToast("info", appState.debugMode ? "调试模式已开�? : "调试模式已关�?, "当前仅影响界面级别的日志记录�?);
        renderCurrentView();
      }
      ...
      root.addEventListener("click", async (event) => {
        ...
        try {
          ...
          if (action === "refresh-data") {
            ...
            renderApp();
            showToast("success", "数据已刷�?, "页面展示的信息已经更新�?);
            return;
          }
          ...
          if (action === "refresh-system-schedule") {
            await loadSchedules();
            rerenderIfView("settings");
            showToast("success", "自动计划已刷�?, "当前计划状态已经更新�?);
            return;
          }
          ...
          if (action === "fill-llm-defaults") {
            ...
            showToast("info", "已填充默认配�?, "您可以按需修改后再保存�?);
            return;
          }
          ...
          if (action === "reset-rules") {
            ...
            showToast("info", "已恢复默认规�?, "别忘了点击保存规则配置�?);
            return;
          }
          ...
          if (action === "test-notification") {
            if (!window.diskClawDesktop?.notify) {
              showToast("warn", "当前环境不支�?, "系统通知仅在 Electron 桌面端可用�?);
              return;
            }
            await window.diskClawDesktop.notify("磁盘清理大虾", "这是一条测试通知�?);
            showToast("success", "通知已发�?, "请检查系统通知区域�?);
            return;
          }
          ...
          if (action === "export-logs") {
            ...
            showToast("success", "日志已导�?, "导出文件已经生成�?);
            return;
          }
          ...
          if (action === "clear-logs") {
            ...
            if (!result.confirmed) {
              showToast("warn", "已取消操�?, "日志没有被清空�?);
              return;
            }
            appState.logs = [];
            renderLogsPanel();
            showToast("success", "日志已清�?, "当前日志列表已经重置�?);
            return;
          }
          ...
          if (action === "reset-local-ui") {
            ...
            if (!result.confirmed) {
              showToast("warn", "已取消操�?, "本地界面设置保持不变�?);
              return;
            }
            resetLocalUi();
            return;
          ...
          if (action === "check-update") {
            showToast("info", "检查更�?, "当前开发版暂未接入自动更新服务�?);
            return;
          }
      ...
      initialize().catch((error) => {
        showRequestErrorToast(error, "初始化失�?);
      });
    })();
  `.replace(/\\`/g, "`").replace(/\\\$\{/g, "${");
}
    ...
    <aside class="sidebar">
      ...
      <div class="sidebar-footer">
        <div class="sidebar-version">
          <div>版本：v2.0 开发中</div>
          <div>技术栈未变更，正在重构 ToC 体验�?/div>
        </div>
        <button class="ghost-link" data-action="check-update">检查更�?/button>
      </div>
    </aside>
    ...
    <div class="dialog-card small">
      <div class="dialog-head">
        <div>
          ...
          <h3 class="dialog-title" id="confirmTitle">请确认操�?/h3>
        </div>
      </div>
      <div class="dialog-body">
        ...
        <div class="story-card">
          ...
          <p class="story-copy" id="confirmImpact">�?/p>
        </div>
        ...
        <div id="confirmKeywordWrap" data-keyword="" style="display:none;">
          <label class="label" for="confirmKeywordInput">请输入确认文�?/label>
          <input id="confirmKeywordInput" placeholder="请输入确认文�?>
        </div>
        <div class="button-row">
